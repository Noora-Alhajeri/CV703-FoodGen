...
LR = 1e-5
SEED = 42
SAVE_DIR = "./instructblip_finetuned_foodcap"
...
class FoodCapDataset(Dataset):
    def __init__(self, split, processor, prompt):
        self.data = split
...
# Freeze all, then unfreeze encoder + last 2 decoder blocks + lm_head
...
for name, param in model.named_parameters():
    if (
        ...
        or any(f"decoder.block.{i}" in name for i in [20, 21])
        or "lm_head" in name
    ):
